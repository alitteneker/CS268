...
class Transformation:
    ...
    def scale(self, scale):
        self.mat = np.matrix([[ scale, 0, 0 ], [ 0, scale, 0 ], [ 0, 0, 1 ]]) * self.mat

    @classmethod
    def triangleSize(cls, p1, p2, p3):
        # Heron's formula for calculating the area of a triangle
        a, b, c = ( np.linalg.norm(p1-p2), np.linalg.norm(p1-p3), np.linalg.norm(p2-p3) )
        s = ( a + b + c ) / 2.0
...
class Image:
    ...
    def _calcCertainty(self, other, votes):
        return votes / float( max( len(self.kp), len(other.kp) ) )
    def _getTransform(self, other, matches):
        myFeats, otherFeats = self._matchesToPoints(other, matches)
        return Transformation( Transformation.buildHomography(myFeats, otherFeats), self._calcCertainty(other, len(matches)), [self.index, other.index] )
    ...
    def findTransform(self, other):
        ...
        if len(matches) < 150:
            return False
        max_combs = min( scipy.misc.comb(len(matches), 4) / 8, 5000 )
        thresh_votes = len(matches) * Image.THRESH_VOTE_PERCENT
        i, best_score, best_trans, best_matches = 0, 20, False, []
        while i < max_combs:
            i += 1
            ...
            if score > best_score:
                best_score, best_trans, best_matches = score, trans, match_votes
                print "\t\t\tAttempt " + str(i) + " : votes " + str(score)
                if best_score >= thresh_votes:
                    break
        if best_trans:
            better_trans = self._getTransform(other, best_matches)
            better_score, better_matches = self._testTransform(trans, other, matches)
            if better_score > best_score:
                best_trans, best_score = better_trans, better_score
            best_trans.certainty = self._calcCertainty(other, best_score)
            print "\t\tSuccess with " + str(len(best_matches)) + " votes (" + str( 100 * float(len(best_matches)) / len(matches) ) + "%, " + str(best_trans.certainty) + "%)"
        return best_trans
...
def findTransformations(images):
    ...
    for image in images[1:]:
        comb = Transformation.searchForPaths(image, images[0], transforms)
        if len(comb):
            combinedTransforms += comb
    print "Found " + str(len(combinedTransforms)) + " combinational transforms"
    transforms += combinedTransforms
    imageTransforms = [ Transformation( np.matrix([[1,0,0],[0,1,0],[0,0,1]],np.dtype(float)), 1, [0,0] ) ]
    for i in range(1, len(images)):
        ...
        for trans in transforms:
            if trans.getSourceIndex() == i and trans.getDestinationIndex() == 0:
                ...
                elif imageTransforms[i].certainty < trans.certainty:
                    imageTransforms[i] = trans
                counter += 1
        if counter == 0:
        ...
        for i, point in enumerate(points):
            ...
            for j in (0, 1):
                ...
                if current[j] > maxP[j]:
                    maxP[j] = current[j]
            points[i] = current
        sizes.append([ index, Transformation.triangleSize(points[0], points[1], points[3]) + Transformation.triangleSize(points[0], points[2], points[3]) ])
    size = np.array([ maxP[0] - minP[0], maxP[1] - minP[1] ])
    if minP[0] < 0 or minP[1] < 0:
        delta = -minP.clip(None, 0)
        for trans in transforms:
            trans.addDelta( delta )
    if size[0] > Image.MAX_MERGE_SIZE or size[1] > Image.MAX_MERGE_SIZE:
        scale = Image.MAX_MERGE_SIZE / float( size.max() )
        size *= scale
        for trans in transforms:
            trans.scale( scale )
    sizes.sort( reverse = True, key = lambda pair: pair[1] )
    canvas = np.zeros( ( size[1], size[0], 3), np.uint8 )
    print "Merged image shape (height, width, channels): " + str(canvas.shape)
    for (i, warped_size) in sizes:
...
def main(inputFile, outputFile):
    ...
    # Identify a transformation for each image on to a common coordinate system
    imageTransforms = findTransformations(images)
    print "Transformations identified for all images"

    # Use the transformations to merge each image in to a composite mosaic image
    mergedImage = mergeImages(images, imageTransforms)
    print "Mosaic image generated"
